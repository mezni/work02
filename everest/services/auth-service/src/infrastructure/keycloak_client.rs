...
impl KeycloakClient {
    ...
    async fn get_backend_token(&self) -> Result<String, DomainError> {
        let params = [
            ...
            (
                "client_id",
                &self.config.keycloak.keycloak_backend_client_id,
            ),
            (
                "client_secret",
                &self.config.keycloak.keycloak_backend_client_secret,
            ),
        ];

        let response = self
            .client
            .post(&self.config.keycloak.backend_token_url())
            .form(&params)
            .send()
            .await
            .map_err(|e| {
    ...
    async fn assign_role_to_user(
        ...
        let assign_url = format!(
            ...
            self.config.keycloak.keycloak_url, self.config.keycloak.keycloak_realm, user_id
        );
    ...
    pub async fn login(
        ...
        let params = [
            ...
            (
                "client_id",
                self.config.keycloak.keycloak_auth_client_id.as_str(),
            ),
            ("username", username),
            ("password", password),
        ];
    ...
    pub async fn change_password(
        ...
        let url = format!(
            ...
            self.config.keycloak.keycloak_url,
            self.config.keycloak.keycloak_realm,
            user_keycloak_id
        );
