...
pub trait UserRepository: Send + Sync {
    ...
    async fn deactivate(&self, user_id: &str, deleted_by: &str) -> AppResult<()>;
...
pub trait RegistrationRepository: Send + Sync {
    ...
    async fn increment_resend_count(&self, registration_id: &str) -> AppResult<()>;
...
pub trait InvitationRepository: Send + Sync {
    ...
    async fn mark_accepted(&self, invitation_id: &str, user_id: &str) -> AppResult<()>;

    async fn mark_cancelled(&self, invitation_id: &str) -> AppResult<()>;
}
