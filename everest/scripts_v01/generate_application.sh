...
echo "Generating Fixed Application Layer..."
...
impl UserApplicationService for UserApplicationServiceImpl {
    async fn create_user(&self, create_user_dto: CreateUserDto) -> Result<UserDto, AppError> {
        ...
        if self.user_repository.find_by_email(&create_user_dto.email).await?.is_some() {
            return Err(AppError::ValidationError("User with this email already exists".to_string()));
        }

        if self.user_repository.find_by_username(&create_user_dto.username).await?.is_some() {
    ...
    async fn list_users_by_company(&self, company_id: Uuid) -> Result<Vec<UserDto>, AppError> {
        let users = self.user_repository.list_by_company(company_id).await?;
...
impl AuthApplicationService for AuthApplicationServiceImpl {
    async fn login(&self, login_dto: LoginDto) -> Result<AuthResponseDto, AppError> {
        ...
        let user = self.user_repository.find_by_keycloak_id(&user_info.sub).await?
...
impl AuditApplicationService for AuditApplicationServiceImpl {
    ...
    async fn get_audit_logs_by_user(&self, user_id: Uuid) -> Result<Vec<AuditLogDto>, AppError> {
        let logs = self.audit_log_repository.find_by_user(user_id).await?;
        
        Ok(logs.iter().map(AuditLogDto::from_entity).collect())
    }

    async fn get_audit_logs_by_company(&self, company_id: Uuid) -> Result<Vec<AuditLogDto>, AppError> {
        let logs = self.audit_log_repository.find_by_company(company_id).await?;
        
        Ok(logs.iter().map(AuditLogDto::from_entity).collect())
    }

    async fn get_recent_audit_logs(&self, limit: u32) -> Result<Vec<AuditLogDto>, AppError> {
        let logs = self.audit_log_repository.list_recent(limit).await?;
...
# Application DTOs with proper derives
...
cat > src/application/dtos/request_dto.rs << 'EOF'
...
# Application Commands (fixed imports)
cat > src/application/commands/create_user_command.rs << 'EOF'
use uuid::Uuid;
...
cat > src/application/commands/login_command.rs << 'EOF'
...
cat > src/application/commands/create_company_command.rs << 'EOF'
use uuid::Uuid;
...
cat > src/application/commands/audit_command.rs << 'EOF'
use uuid::Uuid;
...
# Update main application layer mod file
cat >> src/application/mod.rs << 'EOF'

// Re-export commonly used types
pub use dtos::{
    UserDto, CreateUserDto, UpdateUserDto, UserListDto,
    LoginDto, AuthResponseDto, RefreshTokenDto, RegisterDto,
    CompanyDto, CreateCompanyDto, UpdateCompanyDto, CompanyListDto,
