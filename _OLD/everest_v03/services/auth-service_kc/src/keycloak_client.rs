...
pub trait KeycloakClient: Send + Sync {
    async fn create_user(&self, email: &str, username: &str, password: &str, attributes: Option<HashMap<String, Vec<String>>>) -> AppResult<String>;
...
impl HttpKeycloakClient {
    pub fn new(base_url: String, realm: String, backend_id: String, backend_secret: String, auth_id: String) -> Self {
        Self {
    ...
    fn auth_url(&self) -> String {
        format!("{}/realms/{}/protocol/openid-connect", self.base_url, self.realm)
    }
    ...
    async fn get_admin_token(&self) -> AppResult<String> {
        {
            ...
            if let Some(ref t) = *cache {
                if chrono::Utc::now() < t.expires_at { return Ok(t.token.clone()); }
            }
        }
        ...
        if !resp.status().is_success() {
            ...
            return Err(AppError::AuthenticationError(format!("Admin login failed: {}", body)));
        }

        let data: serde_json::Value = resp.json().await.map_err(|e| AppError::NetworkError(e.to_string()))?;
        let token = data["access_token"].as_str().ok_or(AppError::AuthenticationError("No access token".into()))?.to_string();
...
impl KeycloakClient for HttpKeycloakClient {
    async fn create_user(&self, email: &str, username: &str, password: &str, attributes: Option<HashMap<String, Vec<String>>>) -> AppResult<String> {
        ...
        let body = CreateUserRequest {
            ...
            enabled: true,
            email_verified: true,
            credentials: vec![Credential { kind: "password".into(), value: password.into(), temporary: false }],
            attributes,
        };
        ...
        if !resp.status().is_success() {
            return Err(AppError::KeycloakError(resp.text().await.unwrap_or_default()));
        }

        let loc = resp.headers().get("Location").and_then(|l| l.to_str().ok())
            .ok_or(AppError::KeycloakError("No user ID in Location header".into()))?;
        Ok(loc.split('/').last().unwrap_or_default().to_string())
    }

    async fn get_user(&self, user_id: &str) -> AppResult<KeycloakUser> {
        let token = self.get_admin_token().await?;
        let resp = self.http.get(self.user_endpoint(user_id))
            .bearer_auth(token)
            .send()
            .await
            .map_err(|e| AppError::NetworkError(e.to_string()))?;

        if resp.status() == 404 { return Err(AppError::NotFound("User not found".into())); }
        resp.json::<KeycloakUser>().await.map_err(|e| AppError::NetworkError(e.to_string()))
    }
    ...
    async fn enable_user(&self, user_id: &str) -> AppResult<()> {
        ...
        let resp = self.http.put(self.user_endpoint(user_id))
            .bearer_auth(token)
            .json(&body)
            .send()
            .await
    ...
    async fn assign_role(&self, user_id: &str, role_name: &str) -> AppResult<()> {
        ...
        let role_resp = self.http.get(self.role_endpoint(role_name))
            .bearer_auth(&token)
            .send()
            .await
            .map_err(|e| AppError::NetworkError(e.to_string()))?;
        ...
        if !role_resp.status().is_success() {
            ...
            return Err(AppError::NotFound(format!("Role {} lookup failed: {}", role_name, body)));
        }
        
        let role_data: RoleMapping = role_resp.json().await.map_err(|e| AppError::NetworkError(e.to_string()))?;
    ...
    async fn authenticate(&self, username: &str, password: &str) -> AppResult<TokenResponse> {
        ...
        if !resp.status().is_success() { return Err(AppError::AuthenticationError("Login failed".into())); }
        resp.json::<TokenResponse>().await.map_err(|e| AppError::NetworkError(e.to_string()))
    }
    ...
    async fn refresh_token(&self, refresh_token: &str) -> AppResult<TokenResponse> {
        ...
        if !resp.status().is_success() { return Err(AppError::Unauthorized("Refresh failed".into())); }
        resp.json::<TokenResponse>().await.map_err(|e| AppError::NetworkError(e.to_string()))
    }
    ...
    async fn verify_token(&self, access_token: &str) -> AppResult<serde_json::Value> {
        ...
        let resp = self.http.post(url).form(&params).send().await.map_err(|e| AppError::NetworkError(e.to_string()))?;
        let data: serde_json::Value = resp.json().await.map_err(|e| AppError::NetworkError(e.to_string()))?;
        
        if data["active"].as_bool() == Some(true) { Ok(data) } else { Err(AppError::Unauthorized("Inactive token".into())) }
    }
    ...
    async fn get_user_info(&self, access_token: &str) -> AppResult<serde_json::Value> {
        ...
        if !resp.status().is_success() { return Err(AppError::Unauthorized("Invalid token".into())); }
        resp.json().await.map_err(|e| AppError::NetworkError(e.to_string()))
    }
}