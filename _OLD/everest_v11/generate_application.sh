...
# 4. Fix the application services to use correct JWT parameters
...
pub struct AuthApplicationServiceImpl {
    user_repository: Box<dyn domain::repositories::UserRepository>,
    keycloak_client: infrastructure::auth::keycloak::KeycloakClient,
    jwt_service: infrastructure::auth::jwt::JwtService,
}

impl Clone for AuthApplicationServiceImpl {
    fn clone(&self) -> Self {
        panic!("AuthApplicationServiceImpl should not be cloned. Use Arc instead.")
    }
}
...
pub struct CompanyApplicationServiceImpl {
    company_repository: Box<dyn domain::repositories::CompanyRepository>,
    user_repository: Box<dyn domain::repositories::UserRepository>,
}

impl Clone for CompanyApplicationServiceImpl {
    fn clone(&self) -> Self {
        panic!("CompanyApplicationServiceImpl should not be cloned. Use Arc instead.")
    }
}
...
pub struct AuditApplicationServiceImpl {
    audit_log_repository: Box<dyn domain::repositories::AuditLogRepository>,
}

impl Clone for AuditApplicationServiceImpl {
    fn clone(&self) -> Self {
        panic!("AuditApplicationServiceImpl should not be cloned. Use Arc instead.")
    }
}
