...
pub async fn register_user(
    ...
    // 1. Validate - Note: .0 accesses the inner struct
    body.0.validate()
        .map_err(|e| AppError::ValidationError(e.to_string()))?;

    // 2. Service Initialization
    // Pass the Data (which is an Arc internally) or just the fields needed
    let svc = RegistrationService::new(state.clone()); 
    
    // 3. Execution - Ensure you handle the service's Result with '?'
