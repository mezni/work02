...
BEGIN
    ...
    SELECT 
        ...
        gs.name,
        gs.address,
        ST_Distance(gs.location, ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326)::GEOGRAPHY) AS distance_meters,
    ...
    WHERE ST_DWithin(gs.location, ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326)::GEOGRAPHY, p_radius_meters)
    ORDER BY ST_Distance(gs.location, ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326)::GEOGRAPHY)
    LIMIT p_limit;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(AVG(c.power_kw), 0) AS avg_power_kw,
