...
impl KeycloakClient {
    ...
    /// Fetches an admin access token using client credentials
    pub async fn get_admin_token(&self) -> Result<String, InfrastructureError> {
        ...
        if !status.is_success() {
            return Err(InfrastructureError::KeycloakError(format!(
    ...
    /// Creates a user in Keycloak with role and attributes
    pub async fn create_user(&self, user: &User) -> Result<(), InfrastructureError> {
        ...
        match resp.status() {
            ...
            StatusCode::CONFLICT => Err(InfrastructureError::KeycloakError(
                "User already exists".to_string(),
            )),
            s => {
                let text = resp.text().await.unwrap_or_default();
                Err(InfrastructureError::KeycloakError(format!(
                    "Unexpected response {}: {}",
                    s, text
                )))
            }
